...
# {root_folder_id}/buffer-{env}/{pool_name} for each pool_name passed in below. The id of the bottom level folder
# must be the value of parentFolderId in the buffer service configuration. If root_folder_id is empty, no folders
# will be created.
variable "root_folder_id" {
  type        = string
  description = "Folder under which all projects will be created for this environment."
  default     = ""
}
...
variable "external_folder_ids" {
  type        = list(string)
  description = "List of already existing folders that Buffer SA will get accesss to."
  default     = []
}