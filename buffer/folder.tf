locals {
  create_folders = var.enable && var.parent_folder_id != ""
}

# Create a single folder under the passed in parent folder. This folder will hold the pool-specific folders.
...
resource "google_folder" "pool_folders" {
  for_each     = local.create_folders ? toset(var.pool_ids) : []
