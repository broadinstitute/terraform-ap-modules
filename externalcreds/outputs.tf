...
#
output "ingress_ip" {
  value       = var.enable && var.enable_dns ? google_compute_global_address.ingress_ip[0].address : null
  description = "External Credentials Manager ingress IP"
}
output "ingress_ip_name" {
  value       = var.enable && var.enable_dns ? google_compute_global_address.ingress_ip[0].name : null
  description = "External Credentials Manager ingress IP name"
}
output "fqdn" {
  value       = var.enable && var.enable_dns ? local.fqdn : null
...
output "cloudsql_pg13_outputs" {
  ...
  value = {
    ...
    public_ip = var.enable && local.cloudsql_pg13_settings.enable ? module.cloudsql-pg13.public_ip : null,
    # pg13 CloudSQL instance name
    instance_name = var.enable && local.cloudsql_pg13_settings.enable ? module.cloudsql-pg13.instance_name : null
    # pg13 database root password
    root_user_password = var.enable && local.cloudsql_pg13_settings.enable ? module.cloudsql-pg13.root_user_password : null
    # pg13 app db creds
    app_db_creds = var.enable && local.cloudsql_pg13_settings.enable ? (length(module.cloudsql-pg13.app_db_creds) == 0 ? {} : module.cloudsql-pg13.app_db_creds[local.service]) : null
  }
  sensitive = true
}
