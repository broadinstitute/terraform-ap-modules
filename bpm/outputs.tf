...
output "sqlproxy_sa_id" {
  ...
  description = "BPM Cloud SQL Proxy Google service account ID"
}
output "app_sa_id" {
  value       = length(google_service_account.app) > 0 ? google_service_account.app[0].account_id : null
  description = "BPM App Google service account ID"
}
...
output "cloudsql_pg13_outputs" {
  description = "BPM CloudSQL outputs (pg13 instance)"
  value = {
