...
output "sqlproxy_sa_id" {
  value       = var.enable && !var.preview ? google_service_account.sqlproxy[0].account_id : null
...
output "cloudsql_public_ip" {
  value       = var.enable && !var.preview ? module.cloudsql.public_ip : null
  description = "Workspace Manager CloudSQL instance IP"
}
output "cloudsql_instance_name" {
  value       = var.enable && !var.preview ? module.cloudsql.instance_name : null
  description = "Workspace Manager CloudSQL instance name"
}
output "cloudsql_root_user_password" {
  value       = var.enable && !var.preview ? module.cloudsql.root_user_password : null
...
output "cloudsql_app_db_creds" {
  ...
  value       = var.enable && !var.preview ? (length(module.cloudsql.app_db_creds) == 0 ? {} : module.cloudsql.app_db_creds[local.service]) : null
  description = "Workspace Manager database user credentials"
  sensitive   = true
}
output "cloudsql_app_stairway_db_creds" {
  value       = var.enable && !var.preview ? (length(module.cloudsql.app_db_creds) == 0 ? {} : module.cloudsql.app_db_creds["${local.service}-stairway"]) : null
